...
var Bundle = module.exports = function() {
    ...
    if (arguments.length === 1 && Buffer.isBuffer(arguments[0])) {
        return this.decode(arguments[0])
    }
...
Bundle.prototype.decode = function(data) {
    ...
    bundle._timetag = data.readInt32BE(8);
    offset += 16;
    
    while (offset < data.length) {
        len = data.readInt32BE(offset);
        offset += 4;
        bundle.add(new Message(data.slice(offset, offset + len)));
        offset += len;
    }
...
Bundle.prototype.isBundle = function(data) {
    if (data.length < 8) {
        return false;
    }
    return data.slice(0, 8).toString() === '#bundle\0'
};
